Cypress.Commands.add('fieldsFilling', (data) => {
    cy.get(':nth-child(1) > :nth-child(2) > input').type(data.login);
    cy.get(':nth-child(2) > :nth-child(2) > input').type(data.password);
})

Cypress.Commands.add('fieldsFillingIncorrect', (login, password) => {
    cy.get(':nth-child(1) > :nth-child(2) > input').type(login);
