...
Cypress.Commands.add('dropdown', (url) => {
    ...
    cy.url()
    .should("equal", `${url}/process_purchasetoy.php`);
})

Cypress.Commands.add('makePurchase', (data) => {
    cy.get('#card_nmuber')
    .type(data.cardNumber);
...
Cypress.Commands.add('paymentSuccessfully', (url) => {
    cy.get(':nth-child(2) > h3 > strong')
    .should('be.visible')
    .invoke('text')
    .then((text) => {
        cy.url()
        .should("equal", `${url}/genearte_orderid.php?uid=${text}`);
    });
    ...
    cy.get('td > strong')
    .should('be.visible')
    .contains('Please Note Down Your OrderID');
