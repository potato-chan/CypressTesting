Cypress.Commands.add('addTariffPlansFieldsFilling', (data) => {
    cy.get('#rental1')
    .type(data.monthlyRental);

    cy.get('#local_minutes')
    .type(data.freeLocalMinutes);

    cy.get('#inter_minutes')
    .type(data.freeInternationalMinutes);

    cy.get('#sms_pack')
    .type(data.freeSMSPack);

    cy.get('#minutes_charges')
    .type(data.localPerMinutesCharges);

    cy.get('#inter_charges')
    .type(data.internationalPerMinutesCharges);

    cy.get('#sms_charges')
    .type(data.SMSPerCharges);
})

Cypress.Commands.add('tariffPlanIsAddedChecking', () => {
    cy.location("pathname")
    .should("equal", "/telecom/addtariffplans.php");
