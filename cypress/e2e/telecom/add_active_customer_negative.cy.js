...
describe('Add Customer page', () => {
  ...
  it('KKP-7, The allowed length is exceeded for all text fields', () => {
    ...
    cy.get('#fname').should('not.have.value', data.moreThanMax.firstName);
    cy.get('#lname').should('not.have.value', data.moreThanMax.lastName);
    cy.get('#email').should('not.have.value', data.moreThanMax.email);
    cy.get(':nth-child(7) > #message').should('not.have.value', data.moreThanMax.address);
    cy.get('#telephoneno').should('not.have.value', data.moreThanMax.mobileNumber);
  })
  ...
  it.only('KKP-7, Adding an active customer with empty text fields', () => {
    cy.window().then((win) => {
    ...
    cy.get('#message').should('be.visible').contains(`FirstName ${data.blank}`);
    cy.get('#message50').should('be.visible').contains(`LastName ${data.blank}`);
    cy.get('#message9').should('be.visible').contains(`Email ${data.blank}`);
    cy.get('#message3').should('be.visible').contains(`Address ${data.blank}`);
    cy.get('#message7').should('be.visible').contains(`Mobile number ${data.blank}`);   
  })
})
