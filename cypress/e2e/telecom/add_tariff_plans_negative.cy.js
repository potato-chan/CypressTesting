...
describe('Add Tariff Plans page', () => {
    ...
    it('KKP-4, The allowed length is exceeded for all text fields', () => {
        ...
        cy.get('#rental1')
        .should('not.have.value', data.moreThanMax.monthlyRental);

        cy.get('#local_minutes')
        .should('not.have.value', data.moreThanMax.freeLocalMinutes);

        cy.get('#inter_minutes')
        .should('not.have.value', data.moreThanMax.freeInternationalMinutes);

        cy.get('#sms_pack')
        .should('not.have.value', data.moreThanMax.freeSMSPack);

        cy.get('#minutes_charges')
        .should('not.have.value', data.moreThanMax.localPerMinutesCharges);

        cy.get('#inter_charges')
        .should('not.have.value', data.moreThanMax.internationalPerMinutesCharges);

        cy.get('#sms_charges')
        .should('not.have.value', data.moreThanMax.SMSPerCharges);
    })
    ...
    it('KKP-4, Adding tariff plan with empty text fields', () => {
        ...
        cy.get('#message2')
        .should('be.visible')
        .contains('Number must not be blank');
