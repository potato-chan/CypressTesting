...
describe('Payment Process page', () => {
  ...
  it('KKP-10, The allowed length is exceeded for all text fields', () => {
    ...
    cy.get('#card_nmuber').type(data.moreThanMax.cardNumber);
    cy.get('#card_nmuber').should('not.have.value', data.moreThanMax.cardNumber);
    cy.get('#cvv_code').type(data.moreThanMax.cvv);
    cy.get('#cvv_code').should('not.have.value', data.moreThanMax.cvv);
  })
